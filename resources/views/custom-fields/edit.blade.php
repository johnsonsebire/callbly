                        ...
                        <form method="POST" action="{{ route('custom-fields.update', $customField) }}">
                            ...
                            <div class="row mb-6">
                                <div class="col-lg-6">
                                    ...
                                    <input type="text" name="name" class="form-control form-control-solid @error('name') is-invalid @enderror" 
                                           value="{{ old('name', $customField->label) }}" placeholder="Enter field name" required>
                                    @error('name')
                                ...
                                <div class="col-lg-6">
                                    ...
                                    <select name="field_type" class="form-select form-select-solid @error('field_type') is-invalid @enderror" required>
                                        ...
                                        <option value="text" {{ old('field_type', $customField->type) == 'text' ? 'selected' : '' }}>Text</option>
                                        <option value="number" {{ old('field_type', $customField->type) == 'number' ? 'selected' : '' }}>Number</option>
                                        <option value="email" {{ old('field_type', $customField->type) == 'email' ? 'selected' : '' }}>Email</option>
                                        <option value="phone" {{ old('field_type', $customField->type) == 'phone' ? 'selected' : '' }}>Phone</option>
                                        <option value="date" {{ old('field_type', $customField->type) == 'date' ? 'selected' : '' }}>Date</option>
                                        <option value="select" {{ old('field_type', $customField->type) == 'select' ? 'selected' : '' }}>Select/Dropdown</option>
                                        <option value="textarea" {{ old('field_type', $customField->type) == 'textarea' ? 'selected' : '' }}>Textarea</option>
                                        <option value="checkbox" {{ old('field_type', $customField->type) == 'checkbox' ? 'selected' : '' }}>Checkbox</option>
                                        <option value="url" {{ old('field_type', $customField->type) == 'url' ? 'selected' : '' }}>URL</option>
                                    </select>
                                    @error('field_type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="row mb-6">
                                <div class="col-lg-12">
                                    ...
                                    <textarea name="description" class="form-control form-control-solid @error('description') is-invalid @enderror" 
                                              rows="3" placeholder="Enter field description (optional)">{{ old('description', $customField->description) }}</textarea>
                                    @error('description')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="row mb-6" id="options-container" style="display: none;">
                                <div class="col-lg-12">
                                    ...
                                    <textarea name="options" class="form-control form-control-solid @error('options') is-invalid @enderror" 
                                              rows="5" placeholder="Option 1&#10;Option 2&#10;Option 3">{{ old('options', is_array($customField->options) ? implode("\n", $customField->options) : '') }}</textarea>
                                    @error('options')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="row mb-6">
                                <div class="col-lg-6">
                                    ...
                                    <input type="text" name="default_value" class="form-control form-control-solid @error('default_value') is-invalid @enderror" 
                                           value="{{ old('default_value', $customField->default_value) }}" placeholder="Enter default value (optional)">
                                    @error('default_value')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-lg-6">
                                    ...
                                    <input type="number" name="sort_order" class="form-control form-control-solid @error('sort_order') is-invalid @enderror" 
                                           value="{{ old('sort_order', $customField->sort_order) }}" min="1" placeholder="Sort order">
                                    @error('sort_order')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-lg-6">
                                    <div class="form-check form-switch form-check-custom form-check-solid">
                                        <input class="form-check-input" type="checkbox" name="is_required" id="is_required" 
                                               value="1" {{ old('is_required', $customField->is_required) ? 'checked' : '' }}>
                                ...
                                <div class="col-lg-6">
                                    <div class="form-check form-switch form-check-custom form-check-solid">
                                        <input class="form-check-input" type="checkbox" name="is_active" id="is_active" 
                                               value="1" {{ old('is_active', $customField->is_active) ? 'checked' : '' }}>
                            ...
                            <div class="d-flex justify-content-end">
                                ...
                                <button type="submit" class="btn btn-primary">
                                    <i class="ki-outline ki-check fs-2 me-2"></i> Update Custom Field
                                </button>
                            </div>
                        </form>
                    </div>
